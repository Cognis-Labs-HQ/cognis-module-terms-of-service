...
## 法務ページの Mount 後に Footer リンクを再描画

法務ページは Cognis Page Composer の Mount 完了後に、Footer Link Registry の通知を強制するようになりました。これにより、再読み込み直後の Dashboard から移動した際に、登録済みリンクが表示されないことがある Shell 遷移のタイミング問題を解消します。

## 同意レポートのライブ更新

同意が成功すると直ちに Browser Event が発生し、Mount 中のすべての管理レポートを API から更新します。表示中のレポートは別セッションの同意も5秒ごとに取得し、管理セクションの Unmount 時にすべての Listener と Timer を削除します。Commit リストの項目間に空行が入らないようにしました。
...
同意 Endpoint の 404 応答を定期的に再試行してモジュール復旧後に強制を再開し、Back-Forward Cache からの復元時は有効な Hook を保持し、法務文書を付随するレポート要求より先に読み込みます。レポートの失敗によって公開済みエディター内容が空の定義に置き換わることはありません。
